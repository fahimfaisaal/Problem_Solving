...
int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
   int fine = 0;
    ...
    else if (y1 == y2 && m1 < m2) return fine;
    else if (y1 == y2 && m1 == m2 && d1 < d2) return fine;

    int year = y1 - y2,
        month = m1 - m2,
        day = d1 - d2;

    if (year) return fine = 10000;
    else if (month) return fine = 500 * month;
    else if (day) return fine = 15 * day;
